...
rules/ 파일은 업스트림(leekensei/agent-skills) 원본 그대로다. 이 프로젝트에서는 아래 조정을 적용한 뒤 규칙을 따를 것:

- **수동 메모이제이션 금지**: 규칙 예시에 `useMemo`가 등장해도(예: readability-context-split-by-logic-type) 따라 쓰지 않는다. React Compiler가 켜져 있다 — 함수를 "로직 종류별로 쪼갠다"는 원칙만 취하고 메모이제이션은 컴파일러에 맡긴다.
- **폼 응집도 규칙**(cohesion-form-structure)은 react-hook-form 기준으로 적용한다: 필드 단위 응집 = `register` 검증 옵션, 폼 단위 응집 = resolver. 직접 useState로 폼을 만들지 않는다.
- **서버 상태 예시**는 `useSuspenseQuery` + `AsyncBoundary` 패턴으로 치환해서 적용한다 (apps/web/CLAUDE.md 참조).
- **Props Drilling 제거**(coupling-eliminate-props-drilling) 시 Context 남발 대신, 전역 성격이면 zustand, 서버 데이터면 쿼리 캐시 직접 구독을 먼저 검토한다.
